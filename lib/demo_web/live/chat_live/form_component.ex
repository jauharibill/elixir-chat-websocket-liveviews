defmodule DemoWeb.ChatLive.FormComponent do
  ...
  def update(%{chat: chat} = assigns, socket) do
    changeset = ChatExample.change_chat(chat)
  ...
  def handle_event("validate", %{"chat" => chat_params}, socket) do
    changeset =
      socket.assigns.chat
      |> ChatExample.change_chat(chat_params)
  ...
  defp save_chat(socket, :edit, chat_params) do
    case ChatExample.update_chat(socket.assigns.chat, chat_params) do
      {:ok, _chat} ->
        {:noreply,
         socket
  ...
  defp save_chat(socket, :new, chat_params) do
    case ChatExample.create_chat(chat_params) do
      {:ok, _chat} ->
        {:noreply,
         socket
